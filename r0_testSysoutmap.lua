-- coding:utf-8
-- File Name: r0_testSysoutmap.lua
-- Author: kht
-- Created Time: 2017/5/26 13:43:02
function Initialize(bus_id, ctrl_id)
    adapter.create_fifo(bus_id, ctrl_id)
end

function show_controller_synoutmap(ret_data)
    if adapter.read_elem(ret_data, 0) ~= 0x05 then
        return false
    end

    if adapter.read_elem(ret_data, 2) ~= 0x5 or adapter.read_elem(ret_data, 3) ~= 0 then
        return false
    end
...
function print_controller_synoutmap(bus_id, ctrl_id)
    adapter.clear_upfifo(bus_id, ctrl_id)

    cmd_get_controller_synoutmap = {0x5, 0x4, 0x05, 0x00}
    ret = adapter.write_downdata(bus_id, ctrl_id, cmd_get_controller_synoutmap)
    if not ret then
        return
    end
    ...
    while true do
        ...
        if return_data ~= nil then
            if show_controller_synoutmap(return_data) then
                break
            end
        end
...
Initialize(4, 0)
print_controller_synoutmap(4, 0)
